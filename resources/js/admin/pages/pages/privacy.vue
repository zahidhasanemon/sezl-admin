...
} = await useApi('/page-settings/privacy')
...
const onSubmit = () => {
  refCreateForm.value?.validate().then(async ({ valid: isValid }) => {
    if (isValid) {
      ...
      const { data, error } = await useApi('/page-settings/privacy', {
        method: 'POST',
        body: {
...
<template>
  <VRow>
    ...
    <VCol cols="12">
      <VCard title="Privacy Policy Content">
        <VCardText>
          <!-- 👉 Form -->
          <VForm ref="refCreateForm" @submit.prevent="onSubmit">
            <VRow>
              <VCol cols="12">
                <QuillEditor v-model:content="content" content-type="html" :toolbar="toolbarOptions" theme="snow"
