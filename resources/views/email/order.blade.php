...
<head>
    ...
    <title>VuexyAdmin - Invoice</title>
    <style>
        /* A4 Print Styles */
        @page {
            size: A4;
...
<body>
    <div
        style="max-width: 210mm; margin: 0 auto; background: white; border-radius: 8px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.08);">
        <!-- Gradient Header -->
        ...
        <div class="greeting-box">
            <div class="greeting-text">
                <p>Hello <strong>{{ $order->user->full_name }}</strong>,</p>

                @if ($order->status == 'confirmed')
                    <p>Great news — your order has been <strong>confirmed</strong>! We’ve attached your official invoice
                        below for your records.</p>
                @elseif ($order->status == 'processing')
                    <p>Yay! Your order <strong>#ORD-{{ $order->id }}</strong> has been successfully processed.
                        Here’s
                        your invoice.</p>
                @elseif ($order->status == 'in_transit')
                    <p>Yay! Your order <strong>#ORD-{{ $order->id }}</strong> is on its way! Here’s your
                        invoice.</p>
                @elseif($order->status == 'delivered')
                    <p>Yay! Your order <strong>#ORD-{{ $order->id }}</strong> has been successfully
                        delivered. Here’s
                        your final invoice.</p>
                @elseif($order->payment_status == 'refunded')
                    <p>We’re glad to let you know that order <strong>#ORD-{{ $order->id }}</strong> has been
                        refunded.
                        Refund may take 3–5 business days depending on your payment method.</p>
                @elseif($order->status == 'cancelled')
                    <p>We’re sorry to let you know that order <strong>#ORD-{{ $order->id }}</strong> has been
                        cancelled.
                        @if ($order->payment_status == 'paid')
                            A full refund will be processed to your original payment method within 3–5 business
                            days.
                        @endif
                    </p>
                @else
                    <p>Your order is being processed. Here’s your invoice for reference.</p>
                @endif

                <p style="margin-top: 15px;">If you have any questions, just reply to this email or reach us at
                    <a href="https://mail.google.com/mail/?view=cm&fs=1&to=contact@vuexy-admin.ca" target="_blank"
                        style="color:#667eea;">contact@vuexy-admin.ca</a> — we’re always happy to help!
                </p>

                <p>Warm regards,<br><strong>The VuexyAdmin Team</strong></p>
            </div>
        </div>
    </div>
    <div class="invoice-container" style="margin-top: 10px;">
        @php
            $logoPath = public_path('images/logo-main-black.png');
            $waterMarkPath = public_path('images/circle-logo.png');
            $logo = file_exists($logoPath) ? base64_encode(file_get_contents($logoPath)) : null;
            $waterMark = file_exists($waterMarkPath) ? base64_encode(file_get_contents($waterMarkPath)) : null;
        @endphp
        <!-- Watermark -->
        <!-- <div class="watermark">
            <img src="data:image/png;base64,{{ $waterMark }}" alt="VuexyAdmin Watermark" />
        </div> -->
        <div class="watermark">
            <img src="{{ asset('images/circle-logo.png') }}" alt="VuexyAdmin Watermark" />
        </div>
