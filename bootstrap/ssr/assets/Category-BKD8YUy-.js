...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const search = () => {
      router.get(`/categories/${props.category.slug}`, filters.value, {
        preserveState: true,
        replace: true
      });
    };
    const paginate = (page) => {
      router.get(`/categories/${props.category.slug}`, { ...filters.value, page }, {
        preserveState: true,
        replace: true
      });
    };
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<!--]--><li class="text-gray-800 font-semibold">${ssrInterpolate(__props.category.name)}</li></ol></nav></div></section><section class=""><div class="container"><div class=""><h2 class="text-lg sm:text-xl lg:text-2xl xl:text-3xl font-semibold mb-2">${ssrInterpolate(__props.category.name)}</h2><p class="text-primary-gray">${ssrInterpolate(__props.category.description)}</p></div><div class="my-4 sm:my-6 lg:my-8 bg-[#FFF8F3] p-3 sm:p-4 lg:p-6 rounded-lg lg:rounded-xl"><div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">`);
      if (__props.filterOptions.categories && __props.filterOptions.categories.length && __props.filterOptions.categories[0].children.length) {
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/frontend/Pages/Products/Category.vue");
