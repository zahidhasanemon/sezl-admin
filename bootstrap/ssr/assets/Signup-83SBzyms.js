...
const _sfc_main = /* @__PURE__ */ Object.assign(__default__, {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(
        `<h1 class="text-2xl font-semibold text-primary-black">Welcome to VuexyAdmin</h1></div><div class="login-card"><h2 class="text-xl font-semibold text-primary-black mb-2">Create your account</h2><p class="text-primary-gray text-sm mb-6">Shop faster, track orders, and manage returns.</p><form><div class="grid grid-cols-2 gap-4 mb-4"><div><label for="firstName" class="block text-sm font-medium text-primary-black mb-2">First name</label><input type="text" id="firstName" placeholder="John" class="${ssrRenderClass(
          [{ "border-red-500": unref(form).errors.firstName }, "form-control"]
        )}"${ssrRenderAttr("value", unref(form).firstName)} required>`
      );
      if (unref(form).errors.firstName) {
        _push(
          `<span class="text-red-500 text-sm">${ssrInterpolate(
            unref(form).errors.firstName
          )}</span>`
        );
      } else {
        _push(`<!---->`);
      ...
      _push(
        `</div><div><label for="lastName" class="block text-sm font-medium text-primary-black mb-2">Last name</label><input type="text" id="lastName" placeholder="Doe" class="form-control"${ssrRenderAttr(
          "value",
          unref(form).lastName
        )}>`
      );
      ...
      _push(
        `</div><div><label for="confirmPassword" class="block text-sm font-medium text-primary-black mb-2">Confirm password</label><div class="relative"><span class="absolute left-4 top-1/2 -translate-y-1/2 text-[#11182799]"><svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12.5 7H12V5C12 2.79 10.21 1 8 1C5.79 1 4 2.79 4 5V7H3.5C2.67 7 2 7.67 2 8.5V13.5C2 14.33 2.67 15 3.5 15H12.5C13.33 15 14 14.33 14 13.5V8.5C14 7.67 13.33 7 12.5 7ZM8 11.5C7.17 11.5 6.5 10.83 6.5 10C6.5 9.17 7.17 8.5 8 8.5C8.83 8.5 9.5 9.17 9.5 10C9.5 10.83 8.83 11.5 8 11.5ZM10.1 7H5.9V5C5.9 3.84 6.84 2.9 8 2.9C9.16 2.9 10.1 3.84 10.1 5V7Z" fill="currentColor"></path></svg></span><input${ssrRenderAttr(
          "type",
          isConfirmPasswordVisible.value ? "text" : "password"
        )} id="confirmPassword" placeholder="********" class="form-control !pl-12"${ssrRenderDynamicModel(
          isConfirmPasswordVisible.value ? "text" : "password",
      ...
      if (unref(form).errors.password_confirmation) {
        _push(
          `<span class="text-red-500 text-sm">${ssrInterpolate(
            unref(form).errors.password_confirmation
          )}</span>`
        );
      } else {
        _push(`<!---->`);
      ...
      _push(
        `</div></div><div class="mb-4"><p class="text-xs text-primary-gray mb-2">Password must contain:</p><ul class="text-xs text-primary-gray space-y-1"><li class="flex items-center gap-2"><span class="w-1 h-1 bg-primary-gray rounded-full"></span> At least 8 characters </li><li class="flex items-center gap-2"><span class="w-1 h-1 bg-primary-gray rounded-full"></span> At least one number </li><li class="flex items-center gap-2"><span class="w-1 h-1 bg-primary-gray rounded-full"></span> At least one special character </li></ul></div><div class="mb-6"><label class="flex items-start gap-3 text-sm text-primary-gray cursor-pointer"><input type="checkbox" class="mt-0.5 w-4 h-4 text-primary-black border-gray-300 rounded focus:ring-primary-black"${
          ssrIncludeBooleanAttr(
            Array.isArray(unref(form).terms)
              ? ssrLooseContain(unref(form).terms, null)
              : unref(form).terms
          )
            ? " checked"
            : ""
        } required><span> By creating an account, you agree to our `
      );
      _push(
        ssrRenderComponent(
          unref(link_default),
          ...
          },
          {
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(`Terms and Privacy Policy `);
              } else {
                return [createTextVNode("Terms and Privacy Policy ")];
              }
            }),
            _: 1,
          },
      ...
      _push(
        `. </span></label></div><button type="submit" class="primary-button w-full"${
          ssrIncludeBooleanAttr(validateForm.value) ? " disabled" : ""
        }>Sign up</button><button type="button" class="primary-button-outline w-full mt-4 flex items-center justify-center gap-2"><svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg"><path d="M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844c-.209 1.125-.843 2.078-1.796 2.717v2.258h2.908c1.702-1.567 2.684-3.874 2.684-6.615z" fill="#4285F4"></path><path d="M9.003 18c2.43 0 4.467-.806 5.956-2.18L12.05 13.56c-.806.54-1.836.86-3.047.86-2.344 0-4.328-1.584-5.036-3.711H.96v2.332C2.44 15.983 5.485 18 9.003 18z" fill="#34A853"></path><path d="M3.964 10.712c-.18-.54-.282-1.117-.282-1.71 0-.593.102-1.17.282-1.71V4.96H.957C.347 6.175 0 7.55 0 9.002c0 1.452.348 2.827.957 4.042l3.007-2.332z" fill="#FBBC05"></path><path d="M9.003 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.464.891 11.426 0 9.002 0 5.485 0 2.44 2.017.96 4.958L3.967 7.29c.708-2.127 2.692-3.71 5.036-3.71z" fill="#EA4335"></path></svg> Sign up with Google </button></form></div><p class="text-center text-primary-gray text-sm mt-6"> Already have an account? `
      );
      _push(
        ssrRenderComponent(
          unref(link_default),
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add(
    "resources/js/frontend/Pages/Signup.vue"
  );
